...
import static healpix.FastMath.*;
...
public class FastMathTest extends TestCase {
  private static final double p0 = +0.0,
      m0 = -0.0,
      pinf = Double.POSITIVE_INFINITY,
      minf = Double.NEGATIVE_INFINITY,
      nan = Double.NaN;
  ...
  public void testmain() throws Exception {
    ...
    for (double d = -10; d < 10; d += 0.000001) check1(d);
    for (double d = -10000; d < 10000; d += 0.001) check1(d);

    for (double d = -1; d < 1; d += 0.0000001) check2(d);

    for (double y = -10; y < 10; y += 0.01)
      for (double x = -10; x < 10; x += 0.01) ulptest(atan2(y, x), Math.atan2(y, x), 2.);
    for (double y = -1000; y < 1000; y += 1.01)
      for (double x = -1000; x < 1000; x += 1.01) ulptest(atan2(y, x), Math.atan2(y, x), 2.);
  }
}
