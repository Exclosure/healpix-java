...
public class MocStringIO {
  /**
   * Parses a string following either the basic ASCII or JSON syntax given in the MOC standard
   * document, and converts it into a MOC. The string need not obey the rules for a well-formed MOC
  ...
  public static Moc mocFromString(String in) {
    ...
    for (int i = 0; i < tok.length; ++i) {
      ...
      } else if (tok[i].contains("-")) // range of values
      {
        ...
        ru.add(Long.parseLong(lim[0]) + ofs, Long.parseLong(lim[1]) + ofs + 1);
      } else // single value
    ...
    for (int o = 0; o <= omax; ++o) {
      ...
      if (!rt.isEmpty()) {
        for (int iv = 0; iv < rt.nranges(); ++iv) {
          long a = rt.ivbegin(iv) - offset, b = rt.ivend(iv) - offset;
          if (!prefix) {
            if (!firstOrder) s.append(json ? ", " : " ");
            firstOrder = false;
            if (json) s.append("\"").append(o).append("\":[");
            else s.append(o).append("/");
            prefix = true;
          } else s.append(",");
          if (json) {
            for (long i = a; i < b - 1; ++i) s.append(i).append(",");
            s.append(b - 1);
          } else {
            s.append(a);
  ...
  /**
   * Converts the Moc to its JSON representation as described in the MOC standard document. The
