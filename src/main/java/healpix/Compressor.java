...
/**
 * Class for compressing/uncompressing monotonous integer sequences. Implementation inspired by
 * Moffat and Stuiver 2000: "Binary Interpolative Coding for Effective Index Compression",
 * Information Retrieval 3, 25
 *
...
public class Compressor {
  ...
  private static final class obitstream {
    ...
    public void put(long val, int bits) throws Exception {
      ...
      HealpixUtils.check(val - (val & ((1L << bits) - 1)) == 0, "value has too many bits");
  ...
  private static final class ibitstream {
    ...
    public long get(int bits) throws Exception {
      if (bits == 0) return 0L;
      HealpixUtils.check(bits <= 63, "too many bits");
      HealpixUtils.check((bitpos + bits) <= 8 * (long) data.length, "reading past end of stream");
      long res = 0L;
      return get_internal(res, bits);
    }
  }

  private static void interpol_encode2(long[] data, int l, int r, obitstream obs, int shift)
      throws Exception {
    if (r - l <= 1) return;
    int m = l + (r - l) / 2;
    ...
    if (val < nshort) obs.put(val, nb - 1);
    else obs.put(val + nshort, nb);
  ...
  private static void interpol_decode2(long[] data, int l, int r, ibitstream ibs, int shift)
      throws Exception {
    if (r - l <= 1) return;
    int m = l + (r - l) / 2;
    ...
    if (nval > 1) {
      ...
      long nshort = (1L << nb) - nval;
      val = ibs.get(nb - 1);
      if (val >= nshort) val = (val << 1) + ibs.get(1) - nshort;
    }
