...
/**
 * Class representing a full HEALPix map containing float values. This class is conceptually very
 * similar the the Healpix_Map&lt;float&gt; class of Healpix_cxx.
 *
...
public class HealpixMapFloat extends HealpixBase {
   ...
   * @param scheme_in the new ordering scheme
   */
  public void setDataAndScheme(float[] data_in, Scheme scheme_in) throws Exception {
   ...
   *
   * @param orig map to import
   * @param pessimistic only used when resolution must be reduced: if true, set a pixel to undefined
   *     if at least one the original subpixels was undefined; otherwise only set it to undefined if
   *     all original subpixels were undefined.
   */
  public void importGeneral(HealpixMapFloat orig, boolean pessimistic) throws Exception {
    if (orig.nside == nside) importNograde(orig);
    else if (orig.nside < nside) // upgrading
    importUpgrade(orig);
    else importDegrade(orig, pessimistic);
  }
}
