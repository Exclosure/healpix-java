...
public class MocFitsIO {
  ...
  public static Moc mocFromFits(InputStream inp) throws Exception {
    ...
    long[] data = null;
    if (tmp instanceof long[]) data = (long[]) tmp;
    else if (tmp instanceof int[]) {
      int[] tmp2 = (int[]) tmp;
      data = new long[tmp2.length];
      for (int i = 0; i < tmp2.length; ++i) data[i] = (long) tmp2[i];
    ...
    for (int i = 0; i < data.length; ++i) ru.append(data[i]);
  ...
  public static void mocToFits(Moc moc, OutputStream out) throws Exception {
    ...
    if (maxval <= 0x7fff) {
      ...
      for (int i = 0; i < data.length; ++i) dtmp[i] = (short) data[i];
      table[0] = dtmp;
    } else if (maxval <= 0x7FFFFFFF) {
      int[] dtmp = new int[data.length];
      for (int i = 0; i < data.length; ++i) dtmp[i] = (int) data[i];
      table[0] = dtmp;
    } else table[0] = data;
