...
/**
 * Class for dealing with sets of integer ranges. Ranges are described by the first element and the
 * one-past-last element. This code was inspired by Jan Kotek's "LongRangeSet" class, but has been
...
public class RangeSet {
  ...
  private static final ValueIterator EMPTY_ITER =
      new ValueIterator() {
        public boolean hasNext() {
          return false;
        }
  ...
  /**
   * Checks the object for internal consistency. If a problem is detected, an
   ...
   * @return interval number, starting with -1 (smaller than all numbers in the RangeSet), 0 (first
   *     "on" interval), 1 (first "off" interval etc.), up to (and including) sz-1 (larger than all
   *     numbers in the RangeSet).
   */
  ...
  private static boolean generalAllOrNothing1(
      RangeSet a, RangeSet b, boolean flip_a, boolean flip_b) {
  ...
  private static boolean generalAllOrNothing2(
      RangeSet a, RangeSet b, boolean flip_a, boolean flip_b) {
    int iva = flip_a ? 0 : -1;
    while (iva < a.sz) {
  ...
  private static boolean generalAllOrNothing(
      RangeSet a, RangeSet b, boolean flip_a, boolean flip_b) {
    if (a.isEmpty()) return flip_a ? true : b.isEmpty();
    if (b.isEmpty()) return flip_b ? true : a.isEmpty();
    int strat = strategy(a.nranges(), b.nranges());
    return (strat == 1)
        ? generalAllOrNothing1(a, b, flip_a, flip_b)
        : ((strat == 2)
            ? generalAllOrNothing2(a, b, flip_a, flip_b)
            : generalAllOrNothing2(b, a, flip_b, flip_a));
  }

  /**
   * Internal helper function for constructing unions, intersections and differences of two
   * RangeSets.
   */
  ...
  private static RangeSet generalUnion(RangeSet a, RangeSet b, boolean flip_a, boolean flip_b) {
    if (a.isEmpty()) return flip_a ? new RangeSet() : new RangeSet(b);
    if (b.isEmpty()) return flip_b ? new RangeSet() : new RangeSet(a);
    int strat = strategy(a.nranges(), b.nranges());
    return (strat == 1)
        ? generalUnion1(a, b, flip_a, flip_b)
        : ((strat == 2)
            ? generalUnion2(a, b, flip_a, flip_b)
  ...
  public boolean equals(Object obj) {
    ...
    if ((obj == null) || (!(obj instanceof RangeSet))) return false;
    RangeSet other = (RangeSet) obj;
    if (other.sz != sz) return false;
    for (int i = 0; i < sz; ++i) if (other.r[i] != r[i]) return false;
    return true;
  }

  public int hashCode() {
    int result = Integer.valueOf(sz).hashCode();
    for (int i = 0; i < sz; ++i) result = 31 * result + Long.valueOf(r[sz]).hashCode();
    return result;
  }
  ...
  /**
   * Internal helper function for building unions and differences of the RangeSet with a single
   * range.
   */
  ...
  /**
   * Creates an array containing all the numbers in the RangeSet. Not recommended, because the
   * arrays can become prohibitively large. It is preferrable to use a ValueIterator or explicit
   * loops.
   */
  public long[] toArray() {
    long[] res = new long[(int) nval()];
    int ofs = 0;
    for (int i = 0; i < sz; i += 2) for (long j = r[i]; j < r[i + 1]; ++j) res[ofs++] = j;
    return res;
  }
  ...
  /**
   * Returns a RangeSet obtained by decompressing a byte array which was originally generated by
   * toCompressed().
   */
