...
/**
 * Basic operations related to the HEALPix pixelation. This class is conceptually very similar the
 * the Healpix_Base class of Healpix_cxx. It supports the NESTED scheme for nside parameters which
 * are powers of 2, and the RING scheme for arbitrary positive nside parameters. The maximum
...
public class HealpixBase extends HealpixTables {
  ...
  private Xyf nest2xyf(long ipix) {
    ...
    return new Xyf(compress_bits(pix), compress_bits(pix >>> 1), (int) (ipix >>> (2 * order)));
  }

  private long xyf2nest(int ix, int iy, int face_num) {
    return ((long) (face_num) << (2 * order)) + spread_bits(ix) + (spread_bits(iy) << 1);
  }
  ...
  protected Xyf ring2xyf(long pix) {
    ...
    } else if (pix < (npix - ncap)) // Equatorial region
    {
      ...
      nr = nside;
      long ire = iring - nside + 1, irm = nl2 + 2 - ire;
      long ifm = iphi - (ire >>> 1) + nside - 1, ifp = iphi - (irm >>> 1) + nside - 1;
      if (order >= 0) {
        ifm >>>= order;
        ifp >>>= order;
      } else {
  ...
  protected long xyf2pix(int ix, int iy, int face_num) {
    return (scheme == Scheme.RING) ? xyf2ring(ix, iy, face_num) : xyf2nest(ix, iy, face_num);
  }

  protected long xyf2pix(Xyf xyf) {
    return (scheme == Scheme.RING)
        ? xyf2ring(xyf.ix, xyf.iy, xyf.face)
        : xyf2nest(xyf.ix, xyf.iy, xyf.face);
  }
   ...
   * @return the map order corresponding to {@code nside}; -1 if {@code nside} is not a power of 2.
   */
  ...
  public void setNside(long nside_in) throws Exception {
    ...
    nside = nside_in;
    HealpixUtils.check(nside <= ns_max && nside > 0, "Nside must be between 1 and " + ns_max);
  ...
  public void setNsideAndScheme(long nside_in, Scheme scheme_in) throws Exception {
    ...
    HealpixUtils.check(
        ((nside_in & (nside_in - 1)) == 0) || (scheme_in == Scheme.RING),
  ...
  public HealpixBase() {
    try {
      nside = 0;
      setNsideAndScheme(1, Scheme.NESTED);
    } catch (Exception Ex) {
      /*cannot happen*/
    }
  }
  ...
  public HealpixBase(long nside_in, Scheme scheme_in) throws Exception {
    nside = nside_in - 1;
    setNsideAndScheme(nside_in, scheme_in);
  }
  ...
  protected long loc2pix(Hploc loc) {
    ...
    if (scheme == Scheme.RING) {
      if (za <= Constants.twothird) // Equatorial region
      {
        ...
        long ip = (order > 0) ? (t1 >>> 1) & (nl4 - 1) : ((t1 >>> 1) % nl4); // in {0,4n-1}
    ...
    } else // scheme_ == NEST
    {
      if (za <= Constants.twothird) // Equatorial region
      {
        ...
        long ix = jm & (nside - 1), iy = nside - (jp & (nside - 1)) - 1;
  ...
  protected Hploc pix2loc(long pix) {
    ...
    if (scheme == Scheme.RING) {
      ...
      } else if (pix < (npix - ncap)) // Equatorial region
      {
        long ip = pix - ncap;
        long tmp = (order >= 0) ? ip >>> (order + 2) : ip / nl4;
        long iring = tmp + nside, iphi = ip - nl4 * tmp + 1;
        ;
      ...
      loc.phi =
          (nr == nside)
              ? 0.75 * Constants.halfpi * tmp * fact1
  ...
  /**
   * Returns the neighboring pixels of ipix. This method works in both RING and NEST schemes, but is
   * considerably faster in the NEST scheme.
   *
   * @param ipix the requested pixel number.
   * @return array with indices of the neighboring pixels. The returned array contains (in this
   *     order) the pixel numbers of the SW, W, NW, N, NE, E, SE and S neighbor of ipix. If a
   *     neighbor does not exist (this can only happen for the W, N, E and S neighbors), its entry
   *     is set to -1.
   */
    ...
    if ((ix > 0) && (ix < nsm1) && (iy > 0) && (iy < nsm1)) {
      if (scheme == Scheme.RING)
        ...
        long fpix = (long) (face_num) << (2 * order),
            px0 = spread_bits(ix),
            py0 = spread_bits(iy) << 1,
            pxp = spread_bits(ix + 1),
            pyp = spread_bits(iy + 1) << 1,
            pxm = spread_bits(ix - 1),
            pym = spread_bits(iy - 1) << 1;
      ...
      for (int i = 0; i < 8; ++i) {
        ...
        if (f >= 0) {
          ...
          result[i] = (scheme == Scheme.NESTED) ? xyf2nest(x, y, f) : xyf2ring(x, y, f);
        } else result[i] = -1;
      }
    }
    return result;
   ...
   *
   * @return maximum angular distance between a pixel center and its corners.
   */
   ...
   * This method also accepts the not-really-existing ring indices 0 and 4*Nside, which correspond
   * to North and South Poles, respectively.
   *
   * @param ring ring index: the northernmost ring in the map has index 1; ring indices are
   *     increasing towards the South pole.
   * @return z-coordinate of the ring.
   */
  public double ring2z(long ring) {
    if (ring < nside) return 1 - ring * ring * fact2;
    if (ring <= nl3) return (nl2 - ring) * fact1;
    ring = nl4 - ring;
    return ring * ring * fact2 - 1;
  }
  /**
   * Returns the colatitude for the requested ring. This method also accepts the not-really-existing
   * ring indices 0 and 4*Nside, which correspond to North and South Poles, respectively.
   *
   * @param ring ring index: the northernmost ring in the map has index 1; ring indices are
   *     increasing towards the South pole.
   * @return z-coordinate of the ring.
  ...
  private RangeSet queryStripInternal(double theta1, double theta2, boolean inclusive)
      throws Exception {
    ...
    if (scheme == Scheme.RING) {
      ...
      RingInfoSmall ris1 = get_ring_info_small(ring1), ris2 = get_ring_info_small(ring2);
      long pix1 = ris1.startpix, pix2 = ris2.startpix + ris2.ringpix;
      if (pix1 < pix2) pixset.append(pix1, pix2);
    } else HealpixUtils.check(false, "queryStrip not yet implemented for NESTED");

    return pixset;
  }

  /**
   * Returns a range set of pixels whose centers lie within a given latitude range (if {@code
   * inclusive==false}), or which overlap with this range (if {@code inclusive==true}).
   *
   * <p>The latitude range is defined as follows:
   *
   * <ul>
   *   <li>if {@code theta1<theta2}, it is the range between {@code theta1} and {@code theta2}
   *   <li>otherwise it is the range between 0 and {@code theta2}, and between {@code theta1} and
   *       pi.
   * </ul>
   *
  ...
  private RangeSet queryDiscInternal(Pointing ptg, double radius, int fact) throws Exception {
    ...
    if (scheme == Scheme.RING) {
      int fct = 1;
      if (inclusive) {
        HealpixUtils.check((1L << order_max) / nside >= fact, "invalid oversampling factor");
        fct = fact;
      }
      HealpixBase b2 = null;
      double rsmall, rbig;
      ...
      } else rsmall = rbig = inclusive ? radius + maxPixrad() : radius;
      ...
      for (long iz = irmin; iz <= irmax; ++iz) // rings partially in the disk
      {
        ...
        else dphi = FastMath.atan2(Math.sqrt(ysq), x);
        if (dphi > 0) {
          ...
          long ip_lo = (long) Math.floor(nr * Constants.inv_twopi * (ptg.phi - dphi) - shift) + 1;
          long ip_hi = (long) Math.floor(nr * Constants.inv_twopi * (ptg.phi + dphi) - shift);

          if (fct > 1) {
            while ((ip_lo <= ip_hi)
                && checkPixelRing(b2, ip_lo, nr, ipix1, fct, czphi, cosrsmall, cpix)) ++ip_lo;
            while ((ip_hi > ip_lo)
                && checkPixelRing(b2, ip_hi, nr, ipix1, fct, czphi, cosrsmall, cpix)) --ip_hi;
          }
          ...
          if (ip_lo <= ip_hi) {
            ...
            } else pixset.append(ipix1 + ip_lo, ipix1 + ip_hi + 1);
          }
        }
      }
    ...
    } else // scheme_==NEST
    {
      ...
      double cosrad = FastMath.cos(radius), sinrad = FastMath.sin(radius);
   ...
   *
   * <p>This method is more efficient in the RING scheme.
   *
   ...
   * <p>This method is more efficient in the RING scheme.
   *
   * <p>This method may return some pixels which don't overlap with the polygon at all. The higher
   * {@code fact} is chosen, the fewer false positives are returned, at the cost of increased run
   * time.
   *
   * @param ptg the angular coordinates of the disk center
   * @param radius the radius (in radians) of the disk
   * @param fact The overlapping test will be done at the resolution {@code fact*nside}. For NESTED
   *     ordering, {@code fact} must be a power of 2, else it can be any positive integer. A typical
   *     choice would be 4.
   * @return the requested set of pixel number ranges
   */
  ...
  private RangeSet queryMultiDisc(Vec3[] norm, double[] rad, int fact) throws Exception {
    ...
    if (scheme == Scheme.RING) {
      ...
      if (fct > 1) {
        ...
        rpbig = maxPixrad();
      } else rpsmall = rpbig = inclusive ? maxPixrad() : 0;
      ...
      for (int i = 0; i < nv; ++i) {
        ...
        if (rsmall < Math.PI) {
          ...
          if ((fct > 1) && (rlat2 < Math.PI)) irmax_t = Math.min(nl4 - 1, irmax_t + 1);
      ...
      for (long iz = irmin; iz <= irmax; ++iz) {
        ...
        for (int j = 0; (j < nd) && (rstmp.nranges() > 0); ++j) {
          ...
          double ysq = 1. - z * z - x * x;
          double dphi = (ysq <= 0) ? Math.PI - 1e-15 : FastMath.atan2(Math.sqrt(ysq), x);

          long ip_lo =
              (long) Math.floor(nr * Constants.inv_twopi * (ptg[j].phi - dphi) - shift) + 1;
          long ip_hi = (long) Math.floor(nr * Constants.inv_twopi * (ptg[j].phi + dphi) - shift);

          if (fct > 1) {
            while ((ip_lo <= ip_hi)
                && checkPixelRing(b2, ip_lo, nr, ipix1, fct, czphi[j], cosrsmall[j], cpix[j]))
              ++ip_lo;
            while ((ip_hi > ip_lo)
                && checkPixelRing(b2, ip_hi, nr, ipix1, fct, czphi[j], cosrsmall[j], cpix[j]))
              --ip_hi;
          }
          ...
          if (ip_lo < 0) {
            if (ip_hi + 1 <= ip_lo + nr - 1) rstmp.remove(ipix1 + ip_hi + 1, ipix1 + ip_lo + nr);
          } else rstmp.intersect(ipix1 + ip_lo, ipix1 + ip_hi + 1);
        }
        res.append(rstmp);
      }
    } else // scheme == NEST
    {
      ...
      while (stk.size() > 0) { // as long as there are pixels on the stack
        ...
        for (int i = 0; (i < nv) && (zone > 0); ++i) {
          ...
          for (int iz = 0; iz < zone; ++iz) if (crad < crlimit[o][i][iz]) zone = iz;
        }
  ...
  private RangeSet queryPolygonInternal(Pointing[] vertex, int fact) throws Exception {
    ...
    Vec3[] vv = new Vec3[nv];
    for (int i = 0; i < nv; ++i) vv[i] = new Vec3(vertex[i]);
    ...
    for (int i = 0; i < nv; ++i) {
      ...
      if (i == 0) flip = (hnd < 0.) ? -1 : 1;
      else HealpixUtils.check(flip * hnd > 0, "polygon is not convex");
      normal[i].scale(flip);
    }
    double[] rad = new double[ncirc];
    Arrays.fill(rad, Constants.halfpi);
    if (inclusive) {
      healpix.CircleFinder cf = new CircleFinder(vv);
  ...
  /**
   * Returns a range set of pixels whose centers lie within the convex polygon defined by the {@code
   * vertex} array.
   *
   * <p>This method is more efficient in the RING scheme.
   *
   * @param vertex an array containing the vertices of the requested convex polygon.
   * @return the requested set of pixel number ranges
   */
  ...
  /**
   * Returns a range set of pixels that overlap with the convex polygon defined by the {@code
   * vertex} array.
   *
   * <p>This method is more efficient in the RING scheme.
   *
  ...
  private void check_pixel(
      int o, int omax, int zone, RangeSet pixset, long pix, pstack stk, boolean inclusive) {
    if (zone == 0) return;
    ...
    if (o < order) {
      ...
      for (int i = 0; i < 4; ++i) stk.push(4 * pix + 3 - i, o + 1); // add children
  ...
  /**
   * Returns a set of points along the boundary of the given pixel. Step 1 gives 4 points on the
   * corners. The first point corresponds to the northernmost corner, the subsequent points follow
   * the pixel boundary through west, south and east corners.
   *
