...
public class MocQuery {
  ...
  private static class queryHelper {
    ...
    private void check_pixel(int zone, Moc pixset) {
      if (zone == 0) return;
      if (o < order) {
        if (zone >= 3) pixset.addPixel(o, pix); // output all subpixels
        else // (zone>=1)
      ...
      {
        if (zone >= 2) pixset.addPixel(order, pix);
    ...
    void correctLoc() throws Exception {
      ...
      for (int i = 0; i < nops[myloc]; ++i) correctLoc();
    }
    ...
    int getZone(int zmin, int zmax) {
      ...
      switch (op[myloc]) {
        case AND:
          {
            int z1 = zmax;
            for (int i = 0; i < nops[myloc]; ++i) z1 = getZone(zmin, z1);
            return z1;
          }
        case OR:
          {
            int z1 = zmin;
            for (int i = 0; i < nops[myloc]; ++i) z1 = getZone(z1, zmax);
            return z1;
          }
        case XOR:
          {
            ...
            int z2 = getZone(0, 3);
            return Math.max(
                zmin, Math.min(zmax, Math.max(Math.min(z1, 3 - z2), Math.min(3 - z1, z2))));
          }
        case NOT:
    ...
    public queryHelper(int order_, int omax_, boolean inclusive_, ArrayList<MocQueryComponent> comp)
        throws Exception {
      order = order_;
      omax = omax_;
      ncomp = comp.size();
      ...
      for (o = 0; o <= omax; ++o) // prepare data at the required orders
      {
        ...
        for (int i = 0; i < ncomp; ++i)
          if (op[i] == MocQueryOp.NONE) // it's a cap
          {
            ...
            crmax[o][i] = (r + dr >= Math.PI) ? -1.01 : Math.cos(r + dr);
            crmin[o][i] = (r - dr <= 0.) ? 1.01 : Math.cos(r - dr);
          }
      }
  ...
  public static Moc doMocQueryInclusive(int order, int omax, ArrayList<MocQueryComponent> comp)
      throws Exception {
  ...
  private static int[] getHull(Vec3 vert[], int P[]) throws Exception {
    ...
    for (int i = 3; i < n; i++) { // process the rest of vertices
      ...
      while (isLeft(vert[D[bot]], vert[D[bot + 1]], vert[P[i]]) <= 0) ++bot; // remove bot of deque
      D[--bot] = P[i]; // insert P[i] at bot of deque

      // get the leftmost tangent at the deque top
      while (isLeft(vert[D[top - 1]], vert[D[top]], vert[P[i]]) <= 0) --top; // pop top of deque
    ...
    for (int h = 0; h < nout; h++) res[h] = D[bot + h + 1];

    return res;
  }

  public static ArrayList<MocQueryComponent> prepPolyHelper(
      Vec3 vv[], int P[], ArrayList<MocQueryComponent> comp, boolean doLast) throws Exception {
    ...
    int npockets = 0;
    if (P.length == 3) for (int i = 0; i < 3; i++) addHull[i] = true;
    else {
      do {
        int ihull_next = (ihull + 1) % nhull, ipoly_next = (ipoly + 1) % npoly;
    ...
    if (npockets > 1) comp.add(new MocQueryComponent(MocQueryOp.OR, npockets));
    if (npockets > 0) comp.add(new MocQueryComponent(MocQueryOp.NOT));

    if (!doLast) addHull[hull.length - 1] = false;

    // add convex hull
    for (int i = 0; i < hull.length; ++i)
      if (addHull[i])
        comp.add(
            new MocQueryComponent(
                vv[hull[i]].cross(vv[hull[(i + 1) % hull.length]]).norm(), 0.5 * Math.PI));

    int num_and = 0;
    for (int i = 0; i < hull.length; ++i) if (addHull[i]) ++num_and;
    if (npockets > 0) ++num_and;
    if (num_and > 1) comp.add(new MocQueryComponent(MocQueryOp.AND, num_and));
  ...
  public static ArrayList<MocQueryComponent> prepPolygon(ArrayList<Vec3> vertex) throws Exception {
    ...
    for (int i = 0; i < vertex.size(); ++i) vv[i] = vertex.get(i).norm();
  ...
  public static Moc queryGeneralPolygonInclusive(ArrayList<Vec3> vertex, int order, int omax)
      throws Exception {
