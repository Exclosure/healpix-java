...
public class Moc {
  ...
  public int maxOrder() {
    long combo = 0;
    for (int i = 0; i < rs.nranges(); ++i) combo |= rs.ivbegin(i) | rs.ivend(i);
   ...
   * @param keepPartialCells if true, partially filled cells will be included in the output Moc; if
  ...
  /**
   * @return A RangeSet containing all HEALPix pixels (in NUNIQ order) covered by this Moc. The
   *     result is well-formed in the sense that every pixel is given at its lowest possible HEALPix
   *     order.
   */
  ...
  /**
   * @return A Moc built from the RangeSet of NUNIQ HEALPix pixels given in "ru". "ru" need not be
   *     well-formed.
   */
  ...
  /** @return A compressed representation of the Moc obtained by interpolative coding. */
  ...
  public boolean equals(Object obj) {
    ...
    if ((obj == null) || (!(obj instanceof Moc))) return false;
    Moc other = (Moc) obj;
